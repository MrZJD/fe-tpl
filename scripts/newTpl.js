...
async function starter () {
    ...
    // 检查src目录
    const files = readdirSync(path.resolve(__dirname, '../src'))
    ...
    await git.checkout(branchName, 'master')
    ...
    if (type === 'exists') {
        // -> 从其他模板基础上新建模板
        const source = await myInquirer.list('请选择:', templates.map(item => item.name))
        fromTpl = source
    }
